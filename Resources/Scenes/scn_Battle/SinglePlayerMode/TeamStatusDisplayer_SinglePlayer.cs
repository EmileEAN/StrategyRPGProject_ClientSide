...
{
    public class TeamStatusDisplayer_SinglePlayer : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            try
            {
                ...
                if (m_mainScript.IsInitialized)
                {
                    ...
                    List<GameObject> gos_unit = (playerId == 1) ? m_mainScript.GOs_Player1Unit : m_mainScript.GOs_Player2Unit;
                    m_unitControllers.Clear();
                    for (int i = 0; i < gos_unit.Count; i++)
                    {
                        UnitController_SinglePlayer unitController = gos_unit[i].GetComponent<UnitController_SinglePlayer>();
                        ...
                        goFormatter_unitButton.Format(unit, null, () => m_mainScript.ChangeUnit(unitController.OwnerId, unitController.PrivateIndex), () => m_unitInfoPanelManager.InstantiateInfoPanel(unit, true));
        ...
        {
            foreach (Image image_rarityTexture in m_images_rarityTexture)
            {
                image_rarityTexture.color = Color.white;
            }
        }
    }
}