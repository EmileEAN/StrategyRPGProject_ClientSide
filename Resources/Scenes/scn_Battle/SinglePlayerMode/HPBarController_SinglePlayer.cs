...
{
    public class HPBarController_SinglePlayer : MonoBehaviour
    {
        #region Private Fields
        private UnityBattleSystem_SinglePlayer m_mainScript;

        private UnitController_SinglePlayer m_unitController;
        ...
        void Awake()
        {
            ...
            m_unitController = this.transform.parent.GetComponent<UnitController_SinglePlayer>();
        ...
        private void Initialize()
        {
            try
            {
                ...
                if (m_mainScript.IsInitialized && m_unitController.IsInitialized)
                {
                    ...
                    m_remainingHP = m_unitController.UnitReference.RemainingHP;

                    if (m_remainingHP > 0)
                        m_remainingHP_rate = (float)m_remainingHP / (float)Calculator.MaxHP(m_unitController.UnitReference, m_mainScript.BattleSystemCore);
                    else
        ...
        public void TryUpdateHP(int _amountModified, bool _didHit, bool _isCritical, int _remainingHP)
        {
            ...
            m_remainingHP_rate = m_remainingHP / (float)Calculator.MaxHP(m_unitController.UnitReference, m_mainScript.BattleSystemCore); //Get the ratio of remaining HP over the maximum HP
