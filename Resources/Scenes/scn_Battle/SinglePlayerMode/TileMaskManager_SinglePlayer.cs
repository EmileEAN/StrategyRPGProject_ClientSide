...
{
    public class TileMaskManager_SinglePlayer : MonoBehaviour
    {
        #region Properties
        ...
        private ActionUIManager_SinglePlayer m_actionUIManager;
            ...
            try
            {
                ...
                if (m_mainScript.IsInitialized)
                {
                    ...
                    if (m_actionUIManager == null)
                        m_actionUIManager = GameObject.Find("ActionUI").GetComponent<ActionUIManager_SinglePlayer>();
                    if (m_actionUIManager == null)
                        return;
                ...
                if (!IsInitialized)
                {
                    ...
                    for (int z = 1; z <= SIZE_Z; z++)
                    {
                        for (int x = 1; x <= SIZE_X; x++)
                        {
                            ...
                            Transform transform_tile = this.transform.Find(tileObjectName);
                            MeshRenderer meshRenderer_lowerMask = transform_tile.Find("Mask" + tileNum.ToString()).GetComponent<MeshRenderer>();
                            MeshRenderer meshRenderer_upperMask = transform_tile.Find("UpperMask" + tileNum.ToString()).GetComponent<MeshRenderer>();

                            m_meshRenderers_lowerMask.Add(meshRenderer_lowerMask);
                            m_meshRenderers_upperMask.Add(meshRenderer_upperMask);
                        }
                    }
        ...
        private void ManageMaskVisibility()
        {
            try
            {
                ...
                {
                    foreach (MeshRenderer meshRenderer_lowerMask in m_meshRenderers_lowerMask) { meshRenderer_lowerMask.enabled = false; }
                    foreach (MeshRenderer meshRenderer_upperMask in m_meshRenderers_upperMask) { meshRenderer_upperMask.enabled = false; }
                    m_wasMouseButton1Down = true;
                }
                else if (Input.GetMouseButtonUp(1) && m_wasMouseButton1Down)
                {
                    foreach (MeshRenderer meshRenderer_lowerMask in m_meshRenderers_lowerMask) { meshRenderer_lowerMask.enabled = true; }
                    foreach (MeshRenderer meshRenderer_upperMask in m_meshRenderers_upperMask) { meshRenderer_upperMask.enabled = true; }
                    m_wasMouseButton1Down = false;
                }
            }
            catch (Exception ex)
