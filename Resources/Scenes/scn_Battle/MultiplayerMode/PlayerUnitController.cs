...
namespace EEANWorks.Games.TBSG._01.Unity.SceneSpecific
{
    ...
    {
        #region Properties
        public int PublicIndex { get; private set; }
        public int PrivateIndex { get; private set; } // Units of same Player cannot have same index
        ...
        void Awake()
        {
            ...
            {
                Debug.Log("PlayerUnitController: at Awake()" + ex.Message);
            }
        }
        ...
        private void Initialize()
        {
            try
            {
                ...
                {
                    Debug.Log("PlayerUnitController: Start Initialization.");

                    this.transform.parent = m_mainScript.GO_Player.transform;

                    if (this.transform.parent == null)
                    {
                        Debug.Log("PlayerUnitController: parent object not set");
                        return;
                    }

                    if (!SetImages())
                    {
                        Debug.Log("PlayerUnitController: error at SetImages()");
                        return;
                    }

                    IsInitialized = true;
                    Debug.Log("PlayerUnitController: End Initialization.");
                }
            }
            catch (Exception ex)
            {
                Debug.Log("PlayerUnitController: at Initialize() " + ex.Message);
            }
        }

        public bool SetInitializationData(int _publicIndex, int _privateIndex, int _ownerId, UnitInstance _unitReference, int _maxHP)
        {
        ...
        private bool SetImages()
        {
            ...
            {
                Debug.Log("PlayerUnitController.SetImages() " + ex.Message);
                return false;
            }
        }
    }
